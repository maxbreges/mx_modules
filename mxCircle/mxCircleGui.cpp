...
public:
...

	{
		...
		//if ((!pinORIn) || (!pinMouseOver) || (!pinMouseDown))
		//{
			pinToDSP = false;
	...
	int32_t MP_STDCALL onPointerMove(int32_t flags, GmpiDrawing_API::MP1_POINT point) override
	{
		if (flags & gmpi_gui_api::GG_POINTER_KEY_SHIFT)
		{
			...
			PointL offset(point.x - pointPrevious.x, point.y - pointPrevious.y); // TODO overload subtraction.
			...
			int new_pos = pinAnimPosShift;
			new_pos = new_pos - coarseness * (int)offset.y;
